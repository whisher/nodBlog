...
exports.comment = function(req, res, next, id) {
    ...
    Comment.findById(id).exec(function (err, comment) {
        ...
        if (!comment) {
            return res.jsonp(404,{
                error: 'Failed to load comment ' + id
            });
        }
        req.comment = comment;
        next();
        ...
        if(count > 0){
            comment.save(function(err) {
                ...
                Post.findByIdAndUpdate(comment.post_id, {
                    ...
                    if (!post) {
                        return res.jsonp(404,{
                            error: 'Failed to load post with id ' + comment.post_id
                        });
                    }
            ...
            return res.jsonp(404,{
                error: 'Post with id ' + comment.post_id + 'not found'
            });
        }
    });
};
